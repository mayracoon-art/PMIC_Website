...
433 MHz 頻段的遠程 (LoRa) 技術廣泛應用於無人機遙測資料傳輸。 LoRa 設備在Sleep、Idle和Active三種模式下運行，因此，低靜態電流控制和高動態範圍至關重要。雖然多模式控制可以涵蓋此範圍，但在超輕負載下，轉換效率會受到靜態電流的顯著限制。因此，模式選擇器需同時具備低靜態電流和快速穩定時間，以穩定輸出電壓。本文提出了一種脈衝跳躍負載偵測器，用於在脈衝跳躍自適應導通時間 (PSAOT) 模式和脈衝跳躍非同步模式 (PSAM) 之間切換。負載電流資訊由延遲單元和脈衝數提供，而滯後電流則防止兩種模式之間的振盪。在超輕負載下，寄生體二極體導通負載電流，顯著減少了主動模組的數量。這種全數位邏輯控制實現了低功耗，而具有可變偏移的比較器則改善了瞬態響應。因此，這種高效電源可以覆蓋寬負載範圍，使其適用於資料通訊應用。
對於 GPS 和二氧化碳 (CO2) 感測器而言，優異的轉換效率對於最大限度地減少功率損耗至關重要。雖然常用的 PWM 模式降壓轉換器具有優異的輸出電壓漣波 (OVR) 和效率，但其瞬態性能受到補償網路複雜性的限制。提高瞬態性能通常需要提高開關頻率，但這反過來又會增加開關損耗。為了克服這種權衡，本文提出了一種基於時間的雙模控制方法。此方法採用低功耗邊緣追蹤比較器 (EPC) 來根據電壓差確定工作模式。在穩態下，PWM 模式控制可維持較低的過電壓比 (OVR)。當出現過衝或欠衝時，脈衝相移脈衝序列 (PPS-PT) 被激活，以增強低開關頻率下的瞬態性能並抑制低頻振盪。因此，這種基於時間的控制方法在實現高效瞬態增強的同時，也具有較高的轉換效率。
電子調速器 (ESC) 用於為無刷直流 (BLDC) 馬達供電。在ESC 中，半橋電路是主要的功率損耗來源，這主要是由於高側/低側電晶體的導通損耗所造成的。與矽元件相比，氮化鎵 (GaN) 元件具有高擊穿電壓和低導通電阻 (Ron) 的顯著優勢。然而，GaN 開關也存在反向導通損耗，且缺乏互補開關。為了緩解這些問題，需要採用死區時間控制和自舉電路來降低反向和正向導通損耗。自適應 bang-bang 死區時間控制用於在製程、電壓和溫度 (PVT) 變化期間實現零電壓開關 (ZVS)，從而最大限度地降低低側電晶體的開關損耗。傳統的自舉電路使用高壓電荷共享自舉電容器來降低總電容。然而，這些電容器需要預先計算，並且無法適應由 PCB 寄生電容和不同類型的 GaN 電晶體引起的閘極電荷 (Qg) 變化。此外，高側開關的閘極電壓直接影響 Ron 值。所提出的自適應電荷共享技術（ACSB）將引導電壓維持在最優範圍內範圍，防止閘極過應力，並顯著改善馬達應用中的導通損耗。
飛行控制器 (FC)、相機和無線電控制器 (RC) 對雜訊和干擾敏感。來自直流無刷馬達的強烈干擾會導致不穩定和無人機墜毀。擴頻技術常用於實現低電磁干擾 (EMI)。Gold Code是一種具有偽隨機特性的二元序列，具有優異的自相關與互相關特性，並廣泛用於電信和數據處理的加密。本工作結合這些技術，提出了一種電流模式遲滯升降壓變換器採用雙Gold Code擴頻 (DGCSS)。透過調整遲滯電流窗口和環路延遲，可以增強開關頻率的隨機效應。這可以降低諧波峰值雜散，滿足 CISPR22標準。與傳統的基於漣波的控制相比，額外的補償器改善了負載調節，實現了高精度電壓穩定。
<br><br>
</p>
<img src="/PMSSIC_Lab/assets/images/TT3.png" width="1000px" /> <br>
<p align="justify">
研究聚焦於低功耗生醫感測系統與設計，提出多項適用於早期癌症檢測與高精度量測的關鍵電路與系統整合技術。首先，在早期癌症檢測之單晶片低功耗感測系統方面，本研究提出以CMOS微機電系統（MEMS）為核心的電容式生物感測平台，用於偵測血管內皮生長因子（Vascular Endothelial Growth Factor, VEGF）之濃度變化。感測器採用鍍金指叉電極（Interdigitated Electrode, IDE），並結合去氧核醣核酸（DNA）適體（Aptamer）作為生物辨識元件。當DNA適體與VEGF結合時，會在感測界面形成電雙層電容，其微小電容變化可反映VEGF濃度。為有效擷取此微弱訊號，系統設計低雜訊前端讀出電路，將電容變化轉換至頻率與時間域訊號，以提升抗雜訊能力並降低類比電路複雜度。針對輸入動態範圍寬廣與超低功耗需求，本研究採用兩步式時間數位轉換器（TDC）架構。進一步提出次取樣（Subsampling, SS）技術，以提高轉換增益，抑制雜訊干擾，並克服低供應電壓下解碼器所導致的雜訊底升高問題。此外，系統整合次取樣延遲鎖定迴路（SSDLL）與校正電路，有效提升TDC之線性度與雜訊效能，同時避免校正過程所引入的額外雜訊影響轉換準確度。最終，時間域資訊被高能效地轉換為數位碼，以支援早期癌症分析與臨床應用。
...
2.	**Tsung-Wen Sun**, Chu-En Hsia, Tsung-Heng Tsai", and Chia-Chan Chang, “A 94.3 % Peak Power Efficiency Time-Based Buck Converter Using Pulse-Phase-Shift Modes with An Intrinsic Window for Transient Enhancement,” 2023 20th *2023 20th International SoC Design Conference (ISOCC)*, Oct. 25-28, 2023, Jeju Island, Korea. 2023 IEEE SSCS Seoul Chapter Award. 
